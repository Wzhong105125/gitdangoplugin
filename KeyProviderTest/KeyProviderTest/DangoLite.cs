...
{
    public partial class DangoLite : Form
    {
        char[] Pass = new char[] { '[', '[', '[', '[', '[', '[', '[', '[', '[', '[', '[', '[' };//0~5 First Round, 6~11 Second Round
        bool[] modified = { true, false, false, false, false, false}; //檢驗有無更動
        ...
        int[] color = new int[4];//粉:0 黃:1 灰:2 藍:3 綠:4 白:5 null:6  (黃藍白灰)
        int CurrentRow = 1;
        ...
        public void ShiftColor(int row, int LR)//Test SLL=>LR:0 SRL=>LR:1
        {
            ...
            switch (LR)
            {
             ...
             for(int i = 0; i < 4; i++)
             {
                string pictbox = "pictureBox" + (73 + 4 * row+i);
...
*/
        ...
        public void RandomLoginColor()//將左方之pictureBox著色並記錄
        {
            ...
            for(int i = 73; i <= 96; i+=4)
            {
                int[] ColorInRow = new int[4];
                for (int j = 0; j < 4; j++)
                {
                    ...
                    ColorInRow[dice] = 1;
                    this.CirCle[(i-73)/4,j] = dice; //紀錄Circle之顏色
...
*/
        ...
        private void DangoLite_KeyDown_1(object sender, KeyEventArgs e)//WASD控制方向
        {
            switch (e.KeyCode)
            {
                case Keys.R:
                    for (int i = 0; i < 4; i++)//清空色彩數目
                    {
                        color[i] = 0;
                    }
                    ...
                    CurrentRow = 1;
                    Arrow1.Location = new Point(514, 17+64);
                    RandomColor();
                    GenerateColor();
                    RandomLoginColor();
                    GetPassQ(Pass);
                ...
                case Keys.Enter:
                    IsPassRight();
                    if (check && WhichRound == 1 && CurrentRow == 5)
                    {
                        Close();
                    }
                    else if (!check && WhichRound == 1)
                    {
            ...
            {
                case 0:
                    Arrow1.Location = new Point(514, 17);
                    break;
                case 1:
                    Arrow1.Location = new Point(514, 17+64);
                    break;
                case 2:
                    Arrow1.Location = new Point(514, 17+64*2);
                    break;
                case 3:
                    Arrow1.Location = new Point(514, 17+64*3);
                    break;
                case 4:
                    Arrow1.Location = new Point(514, 17+64*4);
                    break;
                case 5:
                    Arrow1.Location = new Point(514, 17 + 64 * 5);
                    break;
            }
        }
